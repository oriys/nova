...
interface WorkflowDocsProps {
  workflow: Workflow
  currentVersion: WorkflowVersion | null
}

export function WorkflowDocs({ workflow, currentVersion }: WorkflowDocsProps) {
  ...
  const fetchDocs = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const record = await workflowDocsApi.get(workflow.name)
  ...
  const handleGenerate = async () => {
    try {
      ...
      // Build nodes and edges description from the current version
      let nodesDesc = "No published version"
      let edgesDesc = "No edges"
      if (currentVersion) {
        const nodes = currentVersion.nodes || []
        const edges = currentVersion.edges || []
        nodesDesc = nodes.map((n) =>
          `- ${n.node_key}: function="${n.function_name}", timeout=${n.timeout_s}s`
        ).join("\n") || "No nodes"
        const nodeIdMap: Record<string, string> = {}
        for (const n of nodes) nodeIdMap[n.id] = n.node_key
        edgesDesc = edges.map((e) =>
          `- ${nodeIdMap[e.from_node_id] || e.from_node_id} -> ${nodeIdMap[e.to_node_id] || e.to_node_id}`
        ).join("\n") || "No edges"
      }

      const generated = await apiDocsApi.generateWorkflowDocs({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await workflowDocsApi.save(workflow.name, toSave)
      setSavedDocs(toSave)
      setHasChanges(false)
    } catch (err) {
  ...
  const handleDeleteAll = async () => {
    try {
      setError(null)
      await workflowDocsApi.delete(workflow.name)
      setDocs(null)
      setSavedDocs(null)
      setHiddenSections(new Set())
      setHasChanges(false)
