...
export interface AppStoreInstallation {
  id: string;
  tenant_id: string;
  namespace: string;
  app_id: string;
...
export interface AppStoreInstallationResource {
  id: string;
...
export interface AppStoreInstallationDetail {
  installation: AppStoreInstallation;
  resources: AppStoreInstallationResource[];
}

export interface AppStoreInstallJob {
  id: string;
  installation_id: string;
  operation: string;
  status: string;
...
export interface AppStoreResourceConflict {
  resource_type: string;
  resource_name: string;
  existing_id: string;
  reason?: string;
...
export interface AppStorePlannedResource {
  resource_type: string;
  resource_name: string;
  runtime?: string;
  description?: string;
...
export interface AppStoreQuotaCheck {
  ok: boolean;
...
export interface AppStoreInstallPlan {
  valid: boolean;
  conflicts: AppStoreResourceConflict[];
  to_create: AppStorePlannedResource[];
  quota_check: AppStoreQuotaCheck;
  missing_runtimes: string[];
  errors: string[];
}

export interface CreateAppStoreRequest {
  slug: string;
  title: string;
  summary?: string;
  description?: string;
...
export interface PublishReleaseFromResourcesRequest {
  version: string;
  changelog?: string;
...
export interface AppStoreInstallRequest {
  app_slug: string;
  version: string;
  install_name: string;
  name_prefix?: string;
...
export const storeApi = {
  createApp: (data: CreateAppStoreRequest) =>
    request<AppStoreApp>("/store/apps", {
      method: "POST",
      body: JSON.stringify(data),
    }),

  getApp: (slug: string) =>
    request<AppStoreApp>(`/store/apps/${encodeURIComponent(slug)}`),

  deleteApp: (slug: string) =>
    request<void>(`/store/apps/${encodeURIComponent(slug)}`, {
      method: "DELETE",
    }),

  listApps: (params?: { visibility?: AppVisibility; owner?: string; limit?: number; offset?: number }) => {
    ...
    return request<{ apps: AppStoreApp[]; total: number }>(`/store/apps${qs ? `?${qs}` : ""}`);
  },

  publishRelease: (slug: string, payload: { version: string; bundle: File; changelog?: string }) => {
    const form = new FormData();
    form.set("version", payload.version);
    form.set("bundle", payload.bundle);
    if (payload.changelog) {
      form.set("changelog", payload.changelog);
    }
    return request<AppStoreRelease>(`/store/apps/${encodeURIComponent(slug)}/releases`, {
      method: "POST",
      body: form,
    });
  },

  publishReleaseFromResources: (slug: string, payload: PublishReleaseFromResourcesRequest) =>
    request<AppStoreRelease>(`/store/apps/${encodeURIComponent(slug)}/releases`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  getRelease: (slug: string, version: string) =>
    request<AppStoreRelease>(`/store/apps/${encodeURIComponent(slug)}/releases/${encodeURIComponent(version)}`),

  listReleases: (slug: string, limit?: number, offset?: number) => {
    const query = new URLSearchParams();
    if (typeof limit === "number" && Number.isFinite(limit) && limit > 0) {
    ...
    const qs = query.toString();
    return request<{ releases: AppStoreRelease[]; total: number }>(
      `/store/apps/${encodeURIComponent(slug)}/releases${qs ? `?${qs}` : ""}`
    );
  },

  planInstall: (payload: AppStoreInstallRequest) =>
    request<AppStoreInstallPlan>("/store/installations:plan", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  install: (payload: AppStoreInstallRequest) =>
    request<AppStoreInstallResponse>("/store/installations", {
      method: "POST",
      body: JSON.stringify(payload),
    }),

  getInstallation: (id: string) =>
    request<AppStoreInstallationDetail>(`/store/installations/${encodeURIComponent(id)}`),

  listInstallations: (limit?: number, offset?: number) => {
    const query = new URLSearchParams();
    if (typeof limit === "number" && Number.isFinite(limit) && limit > 0) {
      query.set("limit", String(Math.floor(limit)));
    }
    ...
    const qs = query.toString();
    return request<{ installations: AppStoreInstallation[]; total: number }>(
      `/store/installations${qs ? `?${qs}` : ""}`
    );
  },

  uninstall: (id: string, force?: boolean) =>
    request<void>(`/store/installations/${encodeURIComponent(id)}${force ? "?force=true" : ""}`, {
      method: "DELETE",
    }),

  getInstallJob: (id: string) =>
    request<AppStoreInstallJob>(`/store/jobs/${encodeURIComponent(id)}`),
};
