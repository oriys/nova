...
Nova 是一个极简的 Serverless 平台，支持多种虚拟化后端（Firecracker microVM、Apple Virtualization.framework、Docker 等）实现函数级别的隔离执行。每次函数调用都运行在独立的轻量虚拟机或容器中，支持 Python、Go、Rust、WASM、Node.js、Ruby、Java、PHP、Deno、Bun、Lua 等 20+ 运行时。
...
```
用户 (CLI/HTTP)               宿主机                         VM / 容器
   |                            |                              |
   |--- invoke hello -------->  |                              |
   |                      [1] 从 VM 池获取空闲 VM              |
   |                           (没有？创建新 VM)               |
   |                            |                              |
   |                      [2] 启动 VM (Firecracker/Apple VZ/Docker)
   |                            |---- vsock 连接 ----------->  |
   |                            |                        [3] agent 收到请求
   |                            |                            执行 /code/handler
   |                            |<--- 返回 JSON 结果 ------   |
...
2. 调用触发：从 VM 池获取或创建 VM（根据后端选择 Firecracker / Apple VZ / Docker）
...
### 多运行时
- 编译型：Go、Rust、Zig、Swift、C、C++、Java、Kotlin、Scala、GraalVM、WASM
- 解释型：Python、Node.js、Ruby、PHP、Deno、Bun、Lua、Perl、R、Julia、Elixir
...
- 按函数维度管理调度规则（创建、暂停、恢复、删除）

### 事件驱动
- **事件总线**：Topic → Subscription → Delivery 三层模型
- **发布/订阅**：创建 Topic 并发布事件，Subscription 自动将事件分发到目标函数
- **异步调用**：`POST /functions/{name}/invoke-async` 入队异步执行
- **DLQ（死信队列）**：失败超限自动进入死信队列，支持手动重试
- **幂等保证**：基于 idempotency key 的精确一次语义
- **Outbox 模式**：事务性事件发布，保证与业务数据的一致性
...
### 触发器
- 多事件源触发函数执行：Kafka、RabbitMQ、Redis Stream、Webhook、Filesystem
- 触发器状态管理（启用/禁用/健康状态）

### API 网关
- 自定义路由规则（路径 → 函数映射）
- 参数映射（header/query/path → event 字段）
- 路由级限流模板

### 依赖层（Layers）
- 跨函数共享运行时依赖（打包为 Layer）
- 函数可挂载多个 Layer，运行时自动合并到执行环境
...
### 数据库资源（DB Resources）
- 管理数据库连接资源（DSN、凭证策略）
- 函数绑定数据库资源，运行时自动注入连接信息
- 凭证轮换策略 + 访问日志审计

### AI 助手
- 代码生成（`POST /ai/generate`）：根据描述自动生成函数代码
...
| — | nova-vz | macOS VM 管理工具（Apple VZ, CGO） | `cmd/nova-vz/` |
...
```
cmd/{nova,comet,corona,nebula,aurora,zenith} (各服务入口)
    |
    +-- api/server
    |       |-- api/controlplane  (函数 CRUD / 运行时 / 快照 / 配置 / 网关 / AI)
    |       +-- api/dataplane     (调用 / 日志 / 指标 / 健康检查 / 异步)
    |
    +-- executor                  (调用编排)
    |       |-- store             (元数据 + 代码)
    |       |-- pool              (VM 池)
...
在 macOS 上使用 Apple [Virtualization.framework](https://developer.apple.com/documentation/virtualization) 运行原生 Linux VM，通过 Go 工具 `nova-vz`（基于 [Code-Hex/vz](https://github.com/Code-Hex/vz) 绑定）管理 VM 生命周期。

```
Host (macOS)                              Guest (Linux VM)
┌───────────────────────────┐             ┌──────────────────────────┐
│  applevz.Manager          │             │  Agent (PID 1)           │
│                           │             │                          │
│  nova-vz 进程             │  vsock 代理  │  AF_VSOCK port 9999     │
│  (Code-Hex/vz)            │────────────►│                          │
│  UNIX socket              │  VirtioFS   │  代码挂载: /code         │
│  /tmp/nova-vz-*.sock      │────────────►│  (virtiofs, 读写)        │
│                           │             │                          │
│  控制 socket              │             │  Rootfs: ext4 镜像       │
│  save/restore/stop        │             │  (Alpine linux-virt)     │
└───────────────────────────┘             └──────────────────────────┘
```

与 Firecracker 的关键差异：

| 维度 | Firecracker | Apple VZ |
|------|-------------|----------|
| 平台 | Linux x86_64/arm64 + KVM | macOS 13+ (arm64/x86_64) |
| 代码共享 | ext4 代码磁盘 (debugfs 注入) | VirtioFS (共享宿主机目录) |
| 通信 | vsock (AF_VSOCK 直连) | vsock (UNIX socket 代理) |
| 内核 | vmlinux (x86_64 uncompressed) | Image (arm64 uncompressed) |
| 快照 | 内存 + 状态快照 | macOS 14+ save/restore |
| 网络 | TAP + 网桥 + NAT | VirtioNet (可选) |
| 管理工具 | firecracker 二进制 | nova-vz (CGO, 需 codesign) |

**Apple VZ 特有要求：**
- 内核：Alpine linux-virt（需 VIRTIO_PCI 支持），解压为 Image 格式
- Initramfs：需包含 vsock 内核模块（`vsock`, `vmw_vsock_virtio_transport`）
- 构建：`nova-vz` 需要 `CGO_ENABLED=1`，构建后需 codesign（`com.apple.security.virtualization` entitlement）
- Agent 自动检测 VirtioFS 挂载，回退到 `/dev/vdb` 块设备
...
| macOS 虚拟化 | Apple Virtualization.framework | 原生 hypervisor、VirtioFS/vsock、macOS 开发首选 |
...
**Apple VZ 模式（macOS 原生虚拟化）：**
- macOS 13+（Ventura 或更高，快照需 macOS 14+）
- Apple Silicon 或 Intel Mac
- PostgreSQL 14+（Docker 运行）
- arm64 Linux 内核 + initramfs（vsock 模块）+ 运行时 rootfs 镜像
...
```bash
# Docker 模式（全部服务运行在容器内）
make dev          # docker compose up --build（Postgres + 全部服务 + Lumen）
make seed         # 注入示例函数

# macOS 原生模式（服务直接运行在宿主机，支持 Apple VZ 后端）
make dev-native          # 构建 + 启动全部服务（Postgres 仍用 Docker）
make dev-native-status   # 查看服务状态
make dev-native-stop     # 停止全部服务
```

服务端口：Zenith API `:9000`、Nova 控制平面 `:9001`、Comet gRPC `:9090`、Lumen Dashboard `:3000`、PostgreSQL `:5432`

> 原生模式下所有后端检测在宿主机上执行，Apple VZ、Docker、libkrun 等均可正确检测和使用。
...
```
bin/nova-vz                                # Apple VZ 管理工具 (CGO + codesign)
assets/kernel/Image-alpine-arm64           # Alpine linux-virt 内核 (arm64 uncompressed)
assets/kernel/initramfs-alpine-arm64-vsock # Initramfs (含 vsock 内核模块)
assets/rootfs/*-arm64.ext4                 # arm64 运行时 rootfs 镜像
```
...
make nova-vz        # 构建 nova-vz VM 管理工具 (macOS only, CGO + codesign)
```
...
```bash
make rootfs             # Docker 内构建全部 rootfs 镜像 (x86_64)
make rootfs-arm64       # 构建 arm64 rootfs 镜像 (Apple VZ 用)
make rootfs-all         # 构建全部架构 rootfs 镜像
...
| GET | `/functions/{name}/code` | 获取函数代码 |
| PUT | `/functions/{name}/code` | 更新函数代码 |
| GET | `/functions/{name}/files` | 列出函数文件 |
| GET | `/functions/{name}/files/content` | 获取文件内容 |
| GET | `/functions/{name}/versions` | 列出函数版本 |
| GET | `/functions/{name}/versions/{version}` | 获取指定版本 |
| POST | `/functions/{name}/versions/{version}/activate` | 激活版本 |
| GET | `/functions/{name}/versions/{v1}/diff/{v2}` | 版本差异对比 |

### Control Plane — 运行时 / 快照 / 配置 / 后端

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/runtimes` | 列出可用运行时 |
| POST | `/runtimes` | 创建自定义运行时 |
| POST | `/runtimes/upload` | 上传运行时镜像 |
| DELETE | `/runtimes/{id}` | 删除运行时 |
...
| GET | `/topics` | 列出 Topics |
| GET | `/topics/{name}` | 获取 Topic 详情 |
| DELETE | `/topics/{name}` | 删除 Topic |
| POST | `/topics/{name}/publish` | 发布事件 |
| POST | `/topics/{name}/outbox` | 创建 Outbox 条目 |
| GET | `/topics/{name}/outbox` | 列出 Outbox |
| POST | `/topics/{name}/subscriptions` | 创建订阅 |
| GET | `/topics/{name}/subscriptions` | 列出订阅 |
| GET | `/subscriptions/{id}` | 获取订阅详情 |
| PATCH | `/subscriptions/{id}` | 更新订阅 |
| DELETE | `/subscriptions/{id}` | 删除订阅 |
| GET | `/subscriptions/{id}/deliveries` | 列出投递记录 |
| POST | `/subscriptions/{id}/replay` | 重放订阅 |
| POST | `/subscriptions/{id}/seek` | 定位消费位点 |

### Control Plane — 工作流

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/workflows` | 创建 DAG 工作流 |
| GET | `/workflows` | 列出工作流 |
| GET | `/workflows/{name}` | 获取工作流详情 |
| DELETE | `/workflows/{name}` | 删除工作流 |
| POST | `/workflows/{name}/versions` | 发布工作流版本 |
| GET | `/workflows/{name}/versions` | 列出工作流版本 |
| POST | `/workflows/{name}/runs` | 触发工作流运行 |
| GET | `/workflows/{name}/runs` | 列出运行记录 |
| GET | `/workflows/{name}/runs/{runID}` | 获取运行详情 |

### Control Plane — 触发器 / 调度 / 网关

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/triggers` | 创建触发器 |
| GET | `/triggers` | 列出触发器 |
| GET | `/triggers/{id}` | 获取触发器 |
| PATCH | `/triggers/{id}` | 更新触发器 |
| DELETE | `/triggers/{id}` | 删除触发器 |
| POST | `/functions/{name}/schedules` | 创建调度规则 |
| GET | `/functions/{name}/schedules` | 列出调度规则 |
| DELETE | `/functions/{name}/schedules/{id}` | 删除调度规则 |
| PATCH | `/functions/{name}/schedules/{id}` | 切换启用/禁用 |
| POST | `/gateway/routes` | 创建网关路由 |
| GET | `/gateway/routes` | 列出网关路由 |
| PATCH | `/gateway/routes/{id}` | 更新路由 |
| DELETE | `/gateway/routes/{id}` | 删除路由 |

### Control Plane — 安全 / 租户 / RBAC

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/secrets` | 创建 Secret |
| GET | `/secrets` | 列出 Secrets |
| DELETE | `/secrets/{name}` | 删除 Secret |
| POST | `/apikeys` | 创建 API Key |
| GET | `/apikeys` | 列出 API Keys |
| DELETE | `/apikeys/{name}` | 删除 API Key |
| GET | `/tenants` | 列出租户 |
| POST | `/tenants` | 创建租户 |
...
| GET | `/rbac/my-permissions` | 获取当前用户权限 |

### Control Plane — Layers / Volumes / 集群 / AI

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/layers` | 创建 Layer |
| GET | `/layers` | 列出 Layers |
| DELETE | `/layers/{name}` | 删除 Layer |
| PUT | `/functions/{name}/layers` | 设置函数 Layers |
| POST | `/volumes` | 创建 Volume |
| GET | `/volumes` | 列出 Volumes |
| DELETE | `/volumes/{name}` | 删除 Volume |
| POST | `/cluster/nodes` | 注册集群节点 |
| GET | `/cluster/nodes` | 列出集群节点 |
| POST | `/ai/generate` | AI 生成代码 |
| POST | `/ai/review` | AI 审查代码 |
| POST | `/ai/rewrite` | AI 重写代码 |
| GET | `/audit-logs` | 列出审计日志 |

### Control Plane — 数据库资源 / SLO

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/db-resources` | 创建数据库资源 |
| GET | `/db-resources` | 列出数据库资源 |
| POST | `/db-resources/{name}/bindings` | 绑定函数到数据库 |
| PUT | `/functions/{name}/slo` | 设置 SLO 策略 |
| GET | `/functions/{name}/slo` | 获取 SLO 策略 |
| DELETE | `/functions/{name}/slo` | 删除 SLO 策略 |

### Data Plane

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/functions/{name}/invoke` | 同步调用函数 |
| POST | `/functions/{name}/invoke-stream` | 流式调用函数 (SSE) |
| POST | `/functions/{name}/invoke-async` | 异步调用函数 |
| POST | `/functions/{name}/prewarm` | 预热函数 VM |
| GET/PUT/DELETE | `/functions/{name}/state` | 函数级持久状态 |
| GET | `/functions/{name}/logs` | 函数调用日志 |
| GET | `/functions/{name}/logs/stream` | 日志实时流 (SSE) |
| GET | `/functions/{name}/metrics` | 函数维度指标 |
| GET | `/functions/{name}/diagnostics` | 函数诊断信息 |
| POST | `/functions/{name}/diagnostics/analyze` | AI 诊断分析 |
| GET | `/functions/{name}/slo/status` | SLO 达成状态 |
| GET | `/functions/{name}/heatmap` | 函数调用热力图 |
| GET | `/functions/{name}/cost` | 函数成本 |
| GET | `/invocations` | 全局调用日志 |
| GET | `/async-invocations` | 异步调用列表 |
| GET | `/async-invocations/dlq` | 死信队列 |
| POST | `/async-invocations/dlq/retry-all` | 重试全部死信 |
| GET | `/stats` | 池统计 |
| GET | `/metrics` | JSON 格式全局指标 |
| GET | `/metrics/prometheus` | Prometheus 格式指标 |
| GET | `/metrics/timeseries` | 全局时序数据 |
| GET | `/metrics/heatmap` | 全局调用热力图 |
...
### Apple VZ Backend (`internal/applevz/`)

macOS 原生虚拟化后端，使用 Apple Virtualization.framework 运行 Linux VM。通过 `nova-vz`（Go + Code-Hex/vz）管理 VM 生命周期，VirtioFS 共享代码目录，vsock 通信（UNIX socket 代理）。支持快照 save/restore（macOS 14+）。

### Workflow Engine (`internal/workflow/`)

DAG 工作流引擎，支持多步骤函数编排、条件分支、并行执行。工作流以 YAML/JSON 定义，支持版本管理。

### Event Bus (`internal/eventbus/`)

事件总线，实现 Topic → Subscription → Delivery 模型。支持 Outbox 模式（事务性发布）、消费位点管理、投递重试。

### Async Queue (`internal/asyncqueue/`)

异步调用队列，支持自适应并发控制、DLQ（死信队列）、幂等去重。

### Triggers (`internal/triggers/`)

外部事件源触发器：Kafka、RabbitMQ、Redis Stream、Webhook、Filesystem。统一触发器生命周期管理。

### Gateway (`internal/gateway/`)

API 网关，支持自定义路由规则、参数映射（header/query/path → event）、路由级限流模板。

### Layers (`internal/layer/`) & Volumes (`internal/volume/`)

Layer 支持跨函数共享依赖包；Volume 提供持久化存储挂载。

### AI (`internal/ai/`)

集成大语言模型的代码助手，提供生成、审查、重写、诊断分析、测试生成、文档生成等能力。支持自定义 Prompt 模板。

### Cluster (`internal/cluster/`)

多节点集群支持：节点注册/心跳、健康发现、请求路由、放置调度。

### SLO (`internal/slo/`) & Cost (`internal/cost/`)

SLO 策略评估（延迟/成功率阈值）+ 函数级成本计算。

### Auth (`internal/auth/`) & RBAC (`internal/authz/`)

JWT + API Key 双认证模式。RBAC 权限模型（角色 → 权限绑定）。租户级菜单/按钮权限控制。
...
|------|------|------------|------------|---------------------|
| **VM 级隔离** | Firecracker KVM / Apple VZ | Firecracker KVM | V8 Isolate | gVisor |
...
| **认证** | JWT + API Key + RBAC + 速率限制 | IAM + 资源策略 | API tokens | IAM + Invoker 角色 |
| **调度** | Cron 定时触发 | EventBridge 规则 | Cron Triggers | Cloud Scheduler |
| **热更新** | 解释型语言原地 Reload | 需重新部署 | 即时生效 | 需重新部署 |
| **事件源** | HTTP + Cron + Kafka + RabbitMQ + Redis + Webhook | 30+ 事件源 | HTTP + Queue + CRON + Email | 10+ 事件源 |
| **异步调用** | 异步队列 + DLQ + 自动重试 | DLQ + 重试 + 目的地路由 | Tail Workers | DLQ + 重试 |
| **流式响应** | SSE 流式调用 | Response Streaming | 原生 ReadableStream | — |
| **事件总线** | Topic → Subscription + Outbox 模式 | EventBridge | — | Eventarc |
| **工作流** | DAG 工作流引擎 | Step Functions | Workflows (beta) | Cloud Workflows |
| **依赖层** | Layer 跨函数共享 | Lambda Layers | 内置 (npm) | — |
| **持久存储** | Volumes + 函数状态 + DB 资源绑定 | EFS + S3 + DynamoDB | KV + R2 + D1 + Durable Objects | GCS + Firestore |
| **AI 助手** | 代码生成 / 审查 / 重写 / 诊断 / 测试 | CodeWhisperer | AI Gateway | Duet AI |
| **API 网关** | 自定义路由 + 参数映射 + 限流 | API Gateway (REST/HTTP/WS) | 内置 (路由/域名) | API Gateway |
...
> 粗体行 = Nova 已具备的能力，普通行 = Nova 缺失或薄弱的能力。Nova 已实现绝大多数核心 Serverless 能力。

### Nova 的核心优势

1. **极致轻量**：六个静态编译 Go 二进制 + Postgres，零外部依赖（无 Kafka/Redis/etcd）
2. **冷启动快**：Firecracker 快照恢复 + 代码磁盘模板缓存 + singleflight 去重
3. **运行时最广**：20+ 语言支持，自定义运行时，handler-only 风格零样板代码
4. **跨平台后端**：Firecracker (Linux KVM) + Apple VZ (macOS) + Docker (降级)，统一 Agent 协议
5. **可观测性好**：OpenTelemetry trace 穿透 VM 边界 + Prometheus 指标 + 结构化日志 + SLO 评估
6. **热更新**：解释型语言代码更新不销毁 VM，原地 Reload
7. **事件驱动**：内置事件总线 + 异步队列 + DAG 工作流引擎 + 多源触发器
8. **AI 增强**：代码生成、审查、诊断分析、测试生成一体化
...
| 容器镜像部署 | 支持用户直接提交容器镜像 | Lambda Container Image |
...
**为什么在 macOS 上用 Apple Virtualization.framework？**
macOS 不支持 KVM/Firecracker。Apple VZ 提供原生 hypervisor 级隔离，支持 VirtioFS（高效文件共享）和 vsock（低延迟通信），与 Firecracker 共享同一套 Agent 和通信协议，实现跨平台无缝切换。
