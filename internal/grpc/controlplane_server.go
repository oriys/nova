...
import (
	...
	cryptoutil "github.com/oriys/nova/internal/pkg/crypto"
...
func (s *ControlPlaneServer) CreateFunction(ctx context.Context, req *CreateFunctionRequest) (*FunctionInfo, error) {
	...
	if len(req.Code) > 0 {
		...
		sourceHash := cryptoutil.HashString(sourceCode)
		if err := s.store.SaveFunctionCode(ctx, fn.ID, sourceCode, sourceHash); err != nil {
