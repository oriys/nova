...
type Node struct {
	...
	Address       string            `json:"address"` // gRPC address by default; supports http(s):// for HTTP fallback.
...
// HasWarmFunction reports whether this node explicitly advertises warm capacity
// for the given function via labels.
func (n *Node) HasWarmFunction(functionID string) bool {
	if n == nil || functionID == "" || len(n.Labels) == 0 {
		return false
	}

	if v, ok := n.Labels["warm/"+functionID]; ok {
		v = strings.ToLower(strings.TrimSpace(v))
		return v == "1" || v == "true" || v == "yes"
	}

	if raw, ok := n.Labels["warm_functions"]; ok {
		for _, item := range strings.Split(raw, ",") {
			if strings.TrimSpace(item) == functionID {
				return true
			}
		}
	}
