...
func (a *Autoscaler) evaluate() {
	...
	for _, fn := range funcs {
		...
		if policy.ScaleUpThresholds.ColdStartPct > 0 && coldStartRate > policy.ScaleUpThresholds.ColdStartPct {
			scaleUp = true
		}
		if policy.ScaleUpThresholds.AvgLatencyMs > 0 && avgLatencyMs > policy.ScaleUpThresholds.AvgLatencyMs {
			scaleUp = true
		}
