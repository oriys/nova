...
// route's ParamMapping rules and merges them into the function payload.
...
func applyParamMappings(
	...
	// Cache parsed body for body-source lookups
	...
	for _, m := range mappings {
		...
		switch m.Source {
		case domain.ParamSourceQuery:
			if r.URL.Query().Has(m.Name) {
				raw = r.URL.Query().Get(m.Name)
				found = true
			}

		case domain.ParamSourcePath:
			if v, ok := pathParams[m.Name]; ok {
				raw = v
				found = true
			}

		case domain.ParamSourceHeader:
			if v := r.Header.Get(m.Name); v != "" {
				raw = v
				found = true
			}

		case domain.ParamSourceBody:
			if !bodyParsed {
				bodyParsed = true
			...
			if bodyObj != nil {
				if v, ok := bodyObj[m.Name]; ok {
					// Body values are already typed; handle them specially
		...
		// Apply case transformation to the string value
...
func coerceString(s string, t domain.ParamType) (any, error) {
	switch t {
...
func coerceBodyValue(v any, t domain.ParamType, transform domain.ParamTransform) (any, error) {
	// Apply transform if value is a string
	...
	switch t {
	case domain.ParamTypeString:
		// If target is string but source is number/bool, stringify it
		switch val := v.(type) {
		case string:
...
func transformCase(s string, t domain.ParamTransform) string {
	switch t {
	case domain.ParamTransformUpperCase:
		return strings.ToUpper(s)
	case domain.ParamTransformLowerCase:
		return strings.ToLower(s)
	case domain.ParamTransformUpperFirst:
		if s == "" {
			return s
		}
		r := []rune(s)
		...
		return string(r)
	case domain.ParamTransformCamelCase:
		return toCamelCase(s)
	case domain.ParamTransformSnakeCase:
		return toSnakeCase(s)
	case domain.ParamTransformKebabCase:
...
//
//	user_name → userName, content-type → contentType
...
//
//	userName → user_name, HTTPResponse → http_response
...
// splitIdentifier splits on underscore, dash, or camelCase boundaries.
