...
// Client communicates with the nova-agent inside an Apple VZ VM via vsock UNIX socket.
type Client struct {
	vm          *backend.VM
	conn        net.Conn
	mu          sync.Mutex
...
// NewClient creates a new vsock client for the Apple VZ VM.
func NewClient(vm *backend.VM) (*Client, error) {
	if vm.VsockPath == "" {
		return nil, fmt.Errorf("VM %s has no vsock socket path", vm.ID)
	}
...
func (c *Client) dialLocked(timeout time.Duration) error {
	conn, err := net.DialTimeout("unix", c.vm.VsockPath, timeout)
	if err != nil {
		return err
	}
	c.conn = conn
